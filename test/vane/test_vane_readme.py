...
def test_readme(tmp_path, monkeypatch):
    ...
    try:
        ...
        c = ns["connection"]
        assert sorted(
            c.sql("SELECT * FROM read_vortex(?)", params=[ns["files"]]).fetchall()
        ) == [(i, i % 4, f"value-{i}") for i in range(1000)]
        assert sorted(
            c.sql(
                "SELECT * FROM read_vortex(?)", params=[ns["derived_files"]]
            ).fetchall()
        ) == [(i, f"value-{i}") for i in range(900, 1000)]
        for result, key in zip(results, ["files", "derived_files"], strict=True):
            assert result["copy_output_committed"]
            assert sorted(ns[key]) == sorted(x["final_path"] for x in result["files"])
        assert counts["writes"] == 2, counts
